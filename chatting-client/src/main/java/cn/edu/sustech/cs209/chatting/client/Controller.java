...
import cn.edu.sustech.cs209.chatting.common.Post;
...
// 显示新 Stage
  ...
  @FXML
  public void showPostDetail(Message message){
    // 创建一个新的 Stage
    Platform.runLater(new Runnable() {
      @Override
      public void run() {
        ...
        TableColumn<Post, String> idColumn = new TableColumn<>("Id");
        ...
        TableColumn<Post, String> authorNameColumn = new TableColumn<>("Author Name");
        authorNameColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getAuthor()));

        table.getColumns().addAll(idColumn, contentColumn, authorNameColumn);

        ObservableList<Post> data = FXCollections.observableArrayList();
        for (int i = 0; i < message.selectPostId.size(); i++) {
          Post post = new Post(message.selectPostId.get(i), message.selectPostContent.get(i), message.selectPostAuthor.get(i));
          data.add(post);
        }
...
// 显示新 Stage
  ...
  public void PostDetail() {
    Message message = new Message();
    message.setType(MessageType.PostDetail);
    message.setSentBy(username);
    Platform.runLater(new Runnable() {
      @Override
      public void run() {
