...
public class ServerThread implements Runnable {
  ...
  public void getPostDetail(Message message){
    ...
    try {
      ...
      pstmt.setString(1,message.PostName);
      ResultSet rs = pstmt.executeQuery();
      while (rs.next()) {
        s1.add(rs.getString("id"));
        s2.add(rs.getString("post_content"));
        s3.add(rs.getString("post_author"));
      }
      rs.close();
      pstmt.close();
      Message m = new Message();
      m.selectPostId = s1;
      m.selectPostContent = s2;
      m.selectPostAuthor = s3;
      m.setType(MessageType.PostDetail);
      try {
        send(m,s);
      } catch (IOException e) {
        e.printStackTrace();
